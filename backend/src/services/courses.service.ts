...
export class CoursesService {
    ...
    async findUserCourses(userId: string): Promise<CourseResponseDto[]> {
        ...
        return registrations.map((registration) => new CourseResponseDto(registration.course));
    }
    ...
    async enrollUser(userId: string, courseId: string): Promise<void> {
        ...
        if (!existingRegistration) {
            const registration = this.courseRegistrationRepository.create({ userId, courseId });
